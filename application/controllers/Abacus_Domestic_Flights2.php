...
set_time_limit(0);
class Abacus_Domestic_Flights extends CI_Controller {
	...
	public function index()
	{
		...
		if ($this->user['S'] == 1 && $this->user['R'] !=""){
			$data['user'] = $this->user;
			$this->load->view('abacus/index',$data);
		}else {
...
//		if ($this->user['S'] == 1 && $this->user['R'] !=""){
		if ($this->user['S'] == 1 && ( ($this->user['R'] == "1234567") || ($this->user['R'] == "RM852123") || ($this->user['R'] == "1234504") || ($this->user['R'] == "F5880126") )) {
			...
			//SEARCH INDEX
				...
				$airports = json_decode('{"S":1,"M":"Successfuly fetched (42) records","T_DATA":[{"airportname":"Bacolod","airportcode":"BCD"},{"airportname":"Baguio","airportcode":"BAG"},{"airportname":"Basco","airportcode":"BSO"},{"airportname":"Busuanga","airportcode":"USU"},{"airportname":"Butuan","airportcode":"BXU"},{"airportname":"Cagayan De Oro","airportcode":"CGY"},{"airportname":"Camiguin (Mambajao)","airportcode":"CGM"},{"airportname":"Caticlan Malay","airportcode":"MPH"},{"airportname":"Cauayan","airportcode":"CYZ"},{"airportname":"Cebu","airportcode":"CEB"},{"airportname":"Coron","airportcode":"USU"},{"airportname":"Cotabato","airportcode":"CBO"},{"airportname":"Cuyo","airportcode":"CYU"},{"airportname":"Davao","airportcode":"DVO"},{"airportname":"Dumaguete","airportcode":"DGT"},{"airportname":"El Nido","airportcode":"ENI"},{"airportname":"General Santos","airportcode":"GES"},{"airportname":"Iloilo","airportcode":"ILO"},{"airportname":"Kalibo","airportcode":"KLO"},{"airportname":"Laoag","airportcode":"LAO"},{"airportname":"Legaspi","airportcode":"LGP"},{"airportname":"Manila","airportcode":"MNL"},{"airportname":"Marinduque","airportcode":"MRQ"},{"airportname":"Masbate","airportcode":"MBT"},{"airportname":"Naga","airportcode":"WNP"},{"airportname":"Ormoc","airportcode":"OMC"},{"airportname":"Ozamis","airportcode":"OZC"},{"airportname":"Pagadian","airportcode":"PAG"},{"airportname":"Pampanga","airportcode":"CRK"},{"airportname":"Puerto Princesa","airportcode":"PPS"},{"airportname":"Roxas City","airportcode":"RXS"},{"airportname":"Samar, Calbayog","airportcode":"CYP"},{"airportname":"Samar, Catarman","airportcode":"CRM"},{"airportname":"San Jose Mcguire Fd","airportcode":"SJI"},{"airportname":"Surigao","airportcode":"SUG"},{"airportname":"Tablas","airportcode":"TBH"},{"airportname":"Tacloban","airportcode":"TAC"},{"airportname":"Tagbilaran","airportcode":"TAG"},{"airportname":"Tuguegarao","airportcode":"TUG"},{"airportname":"Virac","airportcode":"VRC"},{"airportname":"Zamboanga","airportcode":"ZAM"},{"airportname":"Zamboanga, Dipolog","airportcode":"DPL"}]}',true);
				...
				$this->loadPage(
						'abacus/search/v2/abacus/header_index',
						'abacus/search/v2/abacus/content_index',
						'abacus/search/v2/abacus/footer_index',
						$data
				);
	...
	private function loadPage($customheader,$custompage,$customfooter,$data){
		$this->load->view('abacus/header_open',			$data);
		$this->load->view($customheader,				$data);
		$this->load->view('abacus/header_close',		$data);
		...
		$this->load->view('abacus/footer_open');
		if($customfooter){
			$this->load->view($customfooter);
		}	
		$this->load->view('abacus/footer_close');
...
//			print_r($parameters);
//		echo '</pre>';
		$results['via'] 		= json_decode($this->curl->call($parameters,url),true);

		if($senior == 1){
			$parameters['senior'] = $adult;
			$parameters['adult'] = 1;
		}
		$parameters['actionId'] = 'abacus_ticketing_service/search_flights_domestic_temp2';
		$results['abacus'] 		= json_decode($this->curl->call($parameters,url),true);
...
//		echo '</pre>';
//		echo '<pre>';
//			print_r($results['abacus']);
//		echo '</pre>';

//		$parameters['actionId'] = 'cebupacific_service/cebupacific_search';
//		//$parameters['ip_address'] = '172.16.16.10';
//		$parameters['journey'] = $journey;
//		$results['cebupac'] 	= json_decode($this->curl->call($parameters,url),true);
//		print_r($results['cebupac']);

		if ($results['via'] || $results['abacus']) {
			if ($results['via']['S']==1 || $results['abacus']['S']==1) {
...
//				}
				...
				$passengers = ($results['via']['S']==1)?$results['via']['Passenger']:$results['abacus']['Passenger'];
//				echo '1';
				//print_r($passengers);
...
//							//$senior 	= ($p['Type']=='S')? $adult+1: $adult;
//						}
//						//$_data['via_Passengers'] = array('Adult' => $adult, 'Senior' => 1);
//						//$_data['Passengers'] = array('Adult' => $adult, 'Children' => $children, 'Infant' => $infant, 'Senior' => $adult);
//						//Abacus
//						$_data['Passengers'] = array('Adult' => 0, 'Children' => $children, 'Infant' => $infant, 'Senior' => $adult);
//					}else{
//						$adult 		= 0;
//						$children 	= 0;
//						$infant 	= 0;
...
//							//$senior 	= ($p['Type']=='S')? $adult+1: $adult;
//						}
//						$_data['Passengers'] = array('Adult' => $adult, 'Children' => $children, 'Infant' => $infant, 'Senior' => 0);
//					}
//				echo '<pre>';
//					print_r($_data['Passengers']);
//				echo '</pre>';
//				//}
//
//				//if($results['via']) {
...
////					echo 'via results';
////					if($senior == 1) {
////						$_data['Passengers'] = array('Adult' => $adult, 'Senior' => 1);
////					}else{
////						$adult 		= 0;
////						$children 	= 0;
////						$infant 	= 0;
...
//				//}
			...
			// Add NEW BY SON
				...
				$_data['rqid'] = array(
						'abacus' => $results['abacus']['RQID'],
						'via' => $results['via']['RQID']);

				$_data['Passengers'] = array('Adult' => $adult, 'Children' => $children, 'Infant' => $infant, 'Senior' => $senior);
				$_data['results_onward'] = array(
						'abacus'	=> $results['abacus']['result'][0]['onward'],
						'via' 		=> $results['via']['result'][0]['onward']
				);

				if($results['abacus']['result'][1]['return']||$results['via']['result'][1]['return']){
					$_data['results_return'] = array(
							'abacus'	=> $results['abacus']['result'][1]['return'],
							'via' 		=> $results['via']['result'][1]['return']
					);
				}
				skip_searchAPI:
				$data['data'] = $_data;
				...
				$data['output'] = array(
						'S' => 0,
						'M' => $results['via']['M'].$results['abacus']['M']
				);
			}
		}else{
			$data['process'] = 0;
		...
		$this->loadPage(
				'abacus/search/v2/abacus/header_searchresults',
				'abacus/search/v2/abacus/content_searchresults',
				'abacus/search/v2/abacus/footer_searchresults',
				$data
		);
	}
	...
	private function chooseFlights($provider,$onward_index,$return_index,$_data,$data){
		...
		if($provider=='via'){
			$url = url;
			//$url = str_replace('8002','8003',$url);

			$onward = $this->getFlightsSTR($provider,$_data['results_onward'][$provider][$onward_index]);
			$onward_pricing = $this->getFlightPricingSTR($provider,$_data['results_onward'][$provider][$onward_index]);

			if($return_index>=0){
				$return = $this->getFlightsSTR($provider,$_data['results_return'][$provider][$return_index]);
				$return_pricing = $this->getFlightPricingSTR($provider,$_data['results_return'][$provider][$return_index]);
			}else{
				$return = '';
				$return_pricing = 'currency:,BaseFareFee:,TaxFee:,SystemFee:,TotalFee:';
			}
...
//			print_r($passengers);
			...
			if($return_index>=0){
				$return = $this->getFlightsSTR($provider,$_data['results_return'][$provider][$return_index]);
				$onwardPricingSTR = $this->getFlightPricingSTR($provider,$_data['results_onward'][$provider][$onward_index]);
				$returnPricingSTR = $this->getFlightPricingSTR($provider,$_data['results_return'][$provider][$return_index]);
				//$pricingSTR = $this->getFlightPricingSTR2wayAbacus($_data['results_onward']['abacus'][$onward_index],$_data['results_return']['abacus'][$return_index]);
			}else{
				//$pricingSTR = $this->getFlightPricingSTR($provider,$_data['results_onward'][$provider][$onward_index]);
				$onwardPricingSTR = $this->getFlightPricingSTR($provider,$_data['results_onward'][$provider][$onward_index]);
			}
...
//		print_r($_data);
		...
		//print_r($chosen_onward);
		//print_r($chosen_return);
		//die();
...
//				echo 'via provider';
					...
					$this->loadPage(
							'abacus/search/v2/abacus/header_index',
							'abacus/search/v2/abacus/content_passengerdetails',
							'abacus/search/v2/abacus/footer_passengerdetails',
							$data
					);
...
//		print_r($passengers);
		...
		$this->loadPage(
				'abacus/search/v2/abacus/header_summary',
				'abacus/search/v2/abacus/content_summary',
				'abacus/search/v2/abacus/footer_passengerdetails',
				$data
		);
...
//						echo 'abacus';
						...
						$PNR = (string)$result['PNR'];//$this->curl->call(null,url_mobilereports.'/abacus_getpnrfrom/'.$result['TN']);
						...
						$result = json_decode($this->curl->call($parameters,url),true);*/
						//$result = json_decode('{"S":1,"M":"Success","TKT":["TE 0792315970408-1Y LEE/J U30D*AMP 1656/07DEC*D"],"TN":"GPRSD217ePf16w0"}',true);
						if($result['S']==1){
							$success = true;
						}
					}
...
//					$rqid 			= (string)$_data['rqid']['via'];
		...
		if(!$success){
			...
			$this->loadPage(
					'abacus/search/v2/abacus/header_index',
					'abacus/search/v2/abacus/content_finalpage',
					null,
					$data
			);
	...
	public function search_flights2(){ 
		...
		if ($this->user['S'] == 1 && $this->user['R'] !=""){
			...
			if(isset($_POST['btn_search'])){
				...
				$this->load->view('abacus/header_open',$data);
				$this->load->view('abacus/search/v2/results_header',$data);
				$this->load->view('abacus/header_close',$data);

				$this->load->view('element/top_header',$data);
				$this->load->view('element/wrapper_header');
				$this->load->view('element/left_panel');
				$this->load->view('abacus/search/v2/results',$data);

				$this->load->view('element/wrapper_footer');
				$this->load->view('abacus/footer');
				$this->load->view('abacus/search/v2/results_footer');
				...
				$this->load->view('abacus/header_open', $data);
				$this->load->view('abacus/search/header_index', $data);
				$this->load->view('abacus/header_close', $data);

				$this->load->view('element/top_header', $data);
				$this->load->view('element/wrapper_header');
				$this->load->view('element/left_panel');
				$this->load->view('abacus/search/v2/passengerdetails', $data);

				$this->load->view('element/wrapper_footer');
				$this->load->view('abacus/footer');
				$this->load->view('abacus/search/footer_passengerdetails');
				...
				$this->load->view('abacus/header_open', $data);
				$this->load->view('abacus/search/header_summary', $data);
				$this->load->view('abacus/header_close', $data);

				$this->load->view('element/top_header', $data);
				$this->load->view('element/wrapper_header');
				$this->load->view('element/left_panel');
				$this->load->view('abacus/search/v2/summary', $data);
				...
				$this->load->view('abacus/header_open', $data);
				$this->load->view('abacus/search/header_index');
				$this->load->view('abacus/header_close', $data);

				$this->load->view('element/top_header', $data);
				$this->load->view('element/wrapper_header');
				$this->load->view('element/left_panel');
				$this->load->view('abacus/search/finalpage', $data);
				...
				$this->load->view('abacus/search/v2/index',$data);

				$this->load->view('element/wrapper_footer');
				$this->load->view('abacus/footer');
				$this->load->view('abacus/search/footer_index');
				//$this->load->view('layout/footer');
			}
	...
	public function markup()
	{
		...
		if ($this->user['S'] == 1 && $this->user['R'] !=""){
			...
			$this->load->view('element/wrapper_header');
			$this->load->view('element/left_panel');
			$this->load->view('abacus/markup/markup',$data);
			$this->load->view('element/wrapper_footer');
			$this->load->view('layout/footer');	
		}else {
...
//			$data['field'] = array('Transaction #','PNR #','Passenger','Onward(Flight Number)','Source','Destination','Departure','Arrival','Class','Markup Fee','Total Fee','Baggage Fee');
			...
			$this->load->view('element/wrapper_header');
			$this->load->view('element/left_panel');
			$this->load->view('abacus/issueticket/eticket_logs');
			$this->load->view('element/wrapper_footer');
			$this->load->view('layout/footer');
		}else {
			//$this->session->unset_userdata('user');
	...
	public function issue_ticket(){
		...
		if ($this->user['S'] == 1 && $this->user['R'] !=""){
			...
			if(isset($_POST['btnViewDetails'])){
				...
				$this->load->view('element/wrapper_header');
				$this->load->view('element/left_panel');
				$this->load->view('abacus/issueticket/viewpnr',$data);
				$this->load->view('element/wrapper_footer');
				$this->load->view('layout/footer');
				...
				$this->load->view('element/wrapper_header');
				$this->load->view('element/left_panel');
				$this->load->view('abacus/issueticket/summarypage',$data);
				$this->load->view('element/wrapper_footer');
				$this->load->view('layout/footer');
				...
				$this->load->view('layout/header',$data);
				$this->load->view('abacus/issueticket/header_index');
				$this->load->view('element/top_header');
				$this->load->view('element/wrapper_header');
				$this->load->view('element/left_panel');
				$this->load->view('abacus/issueticket/index',$data);
				$this->load->view('element/wrapper_footer');
				$this->load->view('abacus/footer',$data);
				$this->load->view('abacus/issueticket/footer_index',$data);
			}
