...
set_time_limit(0);
class Merged_ticketing extends CI_Controller {
...
/*	  	$ACC_CONTROL = $this->Sp_model->userprivilege(); 
	...
	public function index()
	{
		...
		if ($this->user['S'] == 1 && $this->user['R'] !=""){
			$data['user'] = $this->user;
			$this->load->view('merged/index',$data);
		}else {
	...
	public function search_flights()
	{
		...
		// if ($this->user['S'] == 1 && ( ($this->user['R'] == "1234567") || ($this->user['R'] == "RM852123") || ($this->user['R'] == "1234504") || ($this->user['R'] == "F5880126") )) {
			...
			//SEARCH INDEX
				...
				$this->loadPage(
						'merged/search/v2/abacus/header_index',
						'merged/search/v2/abacus/content_index',
						'merged/search/v2/abacus/footer_index',
						$data
				);
	...
	private function loadPage($customheader,$custompage,$customfooter,$data){
		$this->load->view('merged/header_open',			$data);
		$this->load->view($customheader,				$data);
		$this->load->view('merged/header_close',		$data);
		...
		$this->load->view('merged/footer_open');
		if($customfooter){
			$this->load->view($customfooter);
		}	
		$this->load->view('merged/footer_close');
	...
	private function searchResults($data){
		...
		$parameters = array(
				...
				'regcode'               =>$this->user['R'],
				'actionId'				=>'merged_ticketing_service/search_flights_domestic',
				'ip_address'			=>$this->ip,
				'origin'                =>$origin,
		...
		$this->loadPage(
				'merged/search/v2/abacus/header_searchresults',
				'merged/search/v2/abacus/content_searchresults',
				'merged/search/v2/abacus/footer_searchresults',
				$data
		);
	}
	...
	private function chooseFlights($provider,$onward_index,$return_index,$_data,$data){
		...
		$parameters = array(
				'dev_id' 			=> $this->global_f->dev_id(),
				'regcode' 			=> $this->user['R'],
				'actionId'			=> 'merged_ticketing_service/pricing_rates_request',
				'ip_address' 		=> $this->ip,
				'requestid' 		=> $requestid,
				'onward' 			=> $onward,
				'return' 			=> $return,
	...
	private function enterPassengerDetails($data){
		...
		if($chosen_onward['provider']=='byaheko'){
			$data['requestid'] = $_data['rqid']['byaheko_RQID'];
		} else {
			$data['requestid'] = $_data['rqid']['RQID'];
		}
...
//				echo 'via provider';
					...
					if($_data['results_onward']['result'][$chosen_onward['index']]['Flights'][0]['Carrier'] == 'Cebu Pacific' && $_data['results_onward']['result'][$chosen_return['index']]['Flights'][0]['Carrier'] == 'Cebu Pacific'){
						$data['OnewayCarrierCP'] = $_data['results_onward']['result'][$chosen_onward['index']]['Flights'][0]['CarrierID'];
					}else if($_data['results_onward']['result'][$chosen_onward['index']]['Flights'][0]['Carrier'] == 'Cebu Pacific'){
						$data['OnewayCarrierCP'] = $_data['results_onward']['result'][$chosen_onward['index']]['Flights'][0]['CarrierID'];
					}else if($_data['results_onward']['result'][$chosen_return['index']]['Flights'][0]['Carrier'] == 'Cebu Pacific'){
						$data['OnewayCarrierCP'] = $_data['results_onward']['result'][$chosen_return['index']]['Flights'][0]['CarrierID'];
					}

					$this->loadPage(
							'merged/search/v2/abacus/header_index',
							'merged/search/v2/abacus/content_passengerdetails',
							'merged/search/v2/abacus/footer_passengerdetails',
							$data
					);
...
//				print_r($_data);
					...
					if($_data['results_onward']['result'][$chosen_onward['index']]['Flights'][0]['Carrier'] == 'Cebu Pacific'){
						$data['OnewayCarrierCP'] = $_data['results_onward']['result'][$chosen_onward['index']]['Flights'][0]['CarrierID'];
					}
...
//				print_r($_data);
	...
	private function summaryPage($data){
		...
		$airlineN = json_decode($this->input->post('airlineN'),true);
		$airlineCP = json_decode($this->input->post('airlineCP'),true);

		// var_dump($form_data);

		$parsed_senioridupload = $this->get_parsed_senioriduploads($form_data);
		$seniorIdImg = $this->input->post('seniorIdImg');

		if ($parsed_senioridupload) {
			$parsed_senioridupload = $parsed_senioridupload;
		} else {
			$parsed_senioridupload = $seniorIdImg;
		}

		// var_dump($parsed_senioridupload);
		...
		$data['seniorIdImg'] 	= $parsed_senioridupload;

		$data['airlineNa'] = $airlineN;
		$data['airlineCP'] = $airlineCP;

		// var_dump($data['seniorIdImg']);

		$this->loadPage(
				'merged/search/v2/abacus/header_summary',
				'merged/search/v2/abacus/content_summary',
				'merged/search/v2/abacus/footer_passengerdetails',
				$data
		);
	...
	private function finalPage($data){
		...
		$seniorIdImg = (string)$this->input->post('seniorIdImg');
		// var_dump("final", $seniorIdImg);

		$airlineNationality	= json_decode($this->input->post('airlineNa'),true);
		$airlineCebuPac	= json_decode($this->input->post('airlineCP'),true);
		...
		$success = false;
			...
			if($this->user['R'] == 'F7997947'){
				$parameters = array(
				   ...
				   'email' 	 		    => $email,
				   'provider' 	 		=> $provider,
				   'seniorIdImg' 	 	=> $seniorIdImg,
				   'SKT'				=> $this->user['SKT'],
				   'nationality' 		=> $airlineNationality['nationality'],
				   'airlinecebu'		=> $airlineCebuPac,
...
//					$rqid 			= (string)$_data['rqid']['via'];
		...
		if(!$success){
			...
			$this->loadPage(
					'merged/search/v2/abacus/header_index',
					'merged/search/v2/abacus/content_finalpage',
					null,
					$data
			);
	...
	public function search_flights2(){ 
		...
		if ($this->user['S'] == 1 && $this->user['R'] !=""){
			...
			if(isset($_POST['btn_search'])){
				...
				$this->load->view('merged/header_open',$data);
				$this->load->view('merged/search/v2/results_header',$data);
				$this->load->view('merged/header_close',$data);

				$this->load->view('element/top_header',$data);
				$this->load->view('element/wrapper_header');
				$this->load->view('element/left_panel');
				$this->load->view('merged/search/v2/results',$data);

				$this->load->view('element/wrapper_footer');
				$this->load->view('merged/footer');
				$this->load->view('merged/search/v2/results_footer');
				...
				$this->load->view('merged/header_open', $data);
				$this->load->view('merged/search/header_index', $data);
				$this->load->view('merged/header_close', $data);

				$this->load->view('element/top_header', $data);
				$this->load->view('element/wrapper_header');
				$this->load->view('element/left_panel');
				$this->load->view('merged/search/v2/passengerdetails', $data);

				$this->load->view('element/wrapper_footer');
				$this->load->view('merged/footer');
				$this->load->view('merged/search/footer_passengerdetails');
				...
				$this->load->view('merged/header_open', $data);
				$this->load->view('merged/search/header_summary', $data);
				$this->load->view('merged/header_close', $data);

				$this->load->view('element/top_header', $data);
				$this->load->view('element/wrapper_header');
				$this->load->view('element/left_panel');
				$this->load->view('merged/search/v2/summary', $data);
				...
				$this->load->view('merged/header_open', $data);
				$this->load->view('merged/search/header_index');
				$this->load->view('merged/header_close', $data);

				$this->load->view('element/top_header', $data);
				$this->load->view('element/wrapper_header');
				$this->load->view('element/left_panel');
				$this->load->view('merged/search/finalpage', $data);
				...
				$this->load->view('merged/search/v2/index',$data);

				$this->load->view('element/wrapper_footer');
				$this->load->view('merged/footer');
				$this->load->view('merged/search/footer_index');
				//$this->load->view('layout/footer');
			}
	...
	public function markup()
	{
		...
		if ($this->user['S'] == 1 && $this->user['R'] !=""){
			...
			$this->load->view('element/wrapper_header');
			$this->load->view('element/left_panel');
			$this->load->view('merged/markup/markup',$data);
			$this->load->view('element/wrapper_footer');
			$this->load->view('layout/footer');	
		}else {
	...
	public function eticketing2($tn=null)
	{
		$data = array('menu' => 8,
	...
	public function eticketing($provider=null, $tn=null)
	{
		$data = array('menu' => 'dom',
				'parent'=>'ABACUS' );
...
//			$data['field'] = array('Transaction #','PNR #','Passenger','Onward(Flight Number)','Source','Destination','Departure','Arrival','Class','Markup Fee','Total Fee','Baggage Fee');
			...
			$this->load->view('element/wrapper_header');
			$this->load->view('element/left_panel');
			$this->load->view('merged/issueticket/eticket_logs',$data);
			$this->load->view('element/wrapper_footer');
			$this->load->view('layout/footer');
		}else {
			//$this->session->unset_userdata('user');
	...
	public function issue_ticket(){
		...
		if ($this->user['S'] == 1 && $this->user['R'] !=""){
			...
			if(isset($_POST['btnViewDetails'])){
				...
				$this->load->view('element/wrapper_header');
				$this->load->view('element/left_panel');
				$this->load->view('merged/issueticket/viewpnr',$data);
				$this->load->view('element/wrapper_footer');
				$this->load->view('layout/footer');
				...
				$this->load->view('element/wrapper_header');
				$this->load->view('element/left_panel');
				$this->load->view('merged/issueticket/summarypage',$data);
				$this->load->view('element/wrapper_footer');
				$this->load->view('layout/footer');
				...
				$this->load->view('layout/header',$data);
				$this->load->view('merged/issueticket/header_index');
				$this->load->view('element/top_header');
				$this->load->view('element/wrapper_header');
				$this->load->view('element/left_panel');
				$this->load->view('merged/issueticket/index',$data);
				$this->load->view('element/wrapper_footer');
				$this->load->view('merged/footer',$data);
				$this->load->view('merged/issueticket/footer_index',$data);
			}
	...
	public function add_newid_payout()
	{
		...
		$RQID = $this->input->post('requestid');
		$NUMS = $this->input->post('number');
		...
		if ($_FILES['file']['error'] == 0){
			...
			if($_FILES['file']['size'] < 2*MB) {
				...
				if (false) { //ftp
					$curtime=time();
					// $image_id = md5($this->user['R'].$curtime) . '_seniorid_web.jpg';
					$image_id = $this->user['R'].'_'.$RQID.'_'.$NUMS. '_seniorid_web.jpg';
					$URL = 'ftp://'.ftp_server_radius.':800/ticketing/seniorid/';
					$attachment = 'ftp://frequest:frequest@'.ftp_server_radius.':800/ticketing/seniorid/'.$image_id;
					...
					{ 
						//if success 				
						$result = array('success'=>1,'M'=>'Successfully Uploaded!','A'=>$attachment);
					...
					$file_location = 'ticketing/seniorid';
	...
	public function get_parsed_senioriduploads($form_data)
	{
		...
		while ($x <= $uploadcount) {
			...
			array_push($parsed_uploads,$form_data['seniorIdImg'.$x]);
