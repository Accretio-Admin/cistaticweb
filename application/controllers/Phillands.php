...
class Phillands extends CI_Controller {
	...
	{	
		...
		if ($this->user['S'] == 1 && $this->user['R'] != "")
        {
            ...
            $this->load->view('phillands/index', $data);
		}
        else 
        {
    ...
    public function phillands()
    {
		...
		{	
			...
			$params = array(
				...
				'actionId' => 'ups_phillands/phillands_payin_cash_ledger',
				'regcode' => $this->user['R'],
				'ip_address' =>$this->ip
			);
			...
			$this->load->view('phillands/phillands'); 
	...
	public function phillands_terms_condition()
	{
		$data = array('menu' => 22, 'parent' => '');
		...
		if ($this->user['S'] == 1 && $this->user['R'] != "")
		{
			...
			$this->load->view('phillands/phillands_terms_condition', $data); 
		}
		else
		{
			$this->session->sess_destroy();
    ...
    function phillands_auto_compute()
    {
        $params = array(
            'dev_id' => $this->global_f->dev_id(),
            'actionId' => 'ups_phillands/maturity_computation',
            'regcode' => $this->user['R'],
            'ip_address' => $this->ip,
    ...
    function phillands_transaction()
    {
        $params = array(
            'dev_id' => $this->global_f->dev_id(),
            'actionId' => 'ups_phillands/payin_transaction',
            'regcode' => $this->user['R'],
            'ip_address' => $this->ip,
            'amount' => $this->input->post('amount'),
            'maturity_type' => $this->input->post('maturity_type'),
    ...
    function payout_transaction()
    {
        $params = array(
            'dev_id' => $this->global_f->dev_id(),
            'actionId' => 'ups_phillands/payout_transaction',
            'regcode' => $this->user['R'],
            'ip_address' => $this->ip,
