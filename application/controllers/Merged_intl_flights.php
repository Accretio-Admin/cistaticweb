...
class Merged_intl_flights extends CI_Controller {
...
//update by nhez 03/21/2017
	   ...
	   // 	$this->session->set_userdata('user',$this->user);
	  	...
	  	// if($this->user['L'] == 1) {
	  	if($this->user['A_CTRL']['intl_booking'] == 0) {
	  		redirect('Main/');
	  	}
		...
		if ($this->user['A_CTRL']['online_booking'] == 1){
			...
			if ($this->user['S'] == 1 && $this->user['R'] !=""){
				...
				if (isset($_POST['btnSearchBooking'])) {
					...
					if ($flighttype == 2) {
						...
						$parameters = array(	
				    				...
				    				'regcode'               =>$this->user['R'],
				    				'actionId'				=>'merged_ticketing_service/search_flights_international',
				    				'ip_address'			=>$this->ip,
				    				'origin'                =>$origin[0],
					    ...
					    $results = json_decode($this->curl->call($parameters,$url),true);
					    	...
					    	if ($results) {
					    		if ($results['S'] == 1) {
									...
									$data['cookie'] = $results['scoot_cookie'];
									$data['process'] = 1;
				...
				if (isset($_POST['btnSelectFlight'])) {
					...
					if($flight_no[0]['pricing']['provider']=='abacus'  || $flight_no[0]['pricing']['provider']=='amadeus'){
						$commission = $flight_no[0]['pricing']['commission'];
					...
					$parameters = array(	
								'dev_id'     			=> $this->global_f->dev_id(),
			    				'regcode'               =>$this->user['R'],
			    				'actionId'				=>'merged_ticketing_service/intlpricing_rates_request',
			    				'ip_address'			=>$this->ip,
			    				'requestid'				=>$requestid,
			    				'onward'				=>$onward,		
			    				'return'				=>$return,
			    				...
			    				'cookie'		 		=> $cookie,
			    				'PHtravelTax' 			=> $flight_no[0]['pricing']['PHtravelTax'],
						...
						$baggages = json_decode($this->curl->call($parameters,$url),true);
					 	...
					 	if ($baggages) {
						 	...
						 	if ($baggages['S'] == 1){
								...
								$data['PHtravelTax']  = $flight_no[0]['pricing']['PHtravelTax'];
				...
				if (isset($_POST['btnBookFlights'])) {
						...
						$others = array($form_data['contactno'],$form_data['street'],$form_data['city'],$form_data['zipcode'],$form_data['flight_details'],$form_data['email']);
						// echo '<pre>';
						// 	print_r($others);
						// echo '</pre>';
						...
						} else if($form_data['provider'] == 'amadeus' || $form_data['provider'] == 'abacus') {
						...
						} else if($form_data['provider'] == 'scoot') {
							$parsed_passengers = $this->International_ticketing_model->get_parsed_passengersScoot($form_data);
							$requestid = json_decode($others[4],true)['rqid'];
						...
						$parameters = array(
							...
							'PHtravelTax'   => $form_data['PHtravelTax'],
						...
						if($form_data['provider'] == 'byaheko') {
							...
							$data['ui_passengers_details'] = $this->International_ticketing_model->ui_passengers_detailsScoot($form_data,$this->session->userdata('baggages'));
				...
				if (isset($_POST['btnConfirm'])) {
					...
					$url = url;
				 	$parameters = array(
				 		'dev_id'     		=> $this->global_f->dev_id(),
						'regcode' 	 		=> $this->user['R'],
						'actionId'	 		=> 'merged_ticketing_service/international_booking_request',
						'ip_address' 		=> $this->ip,
						...
						'PHtravelTax'   	=> $information['PHtravelTax'],
				...
				$this->load->view('element/wrapper_header');
				$this->load->view('element/left_panel');
				$this->load->view('onlinebooking/book_intl_flights'); 
	...
	public function eticketing_international($provider=null, $tn=null)
	{
		if ($this->user['A_CTRL']['online_booking'] == 1){
			$data = array('menu' => 9,
						  'parent'=>'DOMESTIC_FLIGHTS' );
			...
			if ($this->user['S'] == 1 && $this->user['R'] !=""){
				...
				if (isset($_POST['btnSubmitTxn'])){ 
					...
					$parameters = array(
							...
							'actionId' 	 		=> 'ups_byaheko_service/intl_price_change_agree',
							'ip_address' 		=> $this->ip,
							$this->user['SKT'] 	=> md5($this->user['R'].$this->user['SKT'])
					);
					...
					$parameters = array(
							...
							'regcode' 	 		=> $this->user['R'],
							'trackingNumber'  	=> $trackno,
							'actionId' 	 		=> 'ups_byaheko_service/intl_price_change_disagree',
							'ip_address' 		=> $this->ip,
							$this->user['SKT'] 	=> md5($this->user['R'].$this->user['SKT'])
					);
