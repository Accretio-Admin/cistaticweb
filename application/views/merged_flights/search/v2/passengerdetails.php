<div class="mainpanel">
    ...
    <div class="contentpanel" ng-app="myApp" ng-controller="myCtrl" ng-cloak>
        ...
        <?php elseif($data1): ?>
            <?php $p = $provider==0?'abacusflights':'viaflights'; ?>
            <?php $f1 = $data1[$p][0]['onward'][$chosen_onward];?>
            <?php $f2 = $data1[$p][0]['return'][$chosen_return];?>
            <?php $pricing1 = $f1['Pricing']; ?>
            <?php $pricing2 = $f2['Pricing']; ?>
            <?php $totalfee = $pricing1['TotalFee'] + $pricing2['TotalFee']; ?>
            <?php $ff = array($f1,$f2); ?>
            <div class="list-group col-md-12">
                <div class="list-group-item">
                    ...
                    <div class="text-right" style="font-size: 20px;"><b style="color:#03A9F4;">TOTAL FEE : {{<?php echo strlen($totalfee)>0?$totalfee:'0'; ?>|currency: "₱ "}}</b></div>
                    <div>
                        <table class="table">
                            <thead>
                            ...
                            <tbody>
                            <?php foreach($ff as $f_i => $f): ?>
                                <?php foreach($f['Breakdown'] as $bd): ?>
                                    <tr>
                                        ...
                                        <td class="text-center" style="font-size:10px; vertical-align: middle;"><?php echo $f_i==0?'ONWARD':'RETURN';?></td>
            ...
            <div class="list-group col-md-12">
                <div class="list-group-item">
                    ...
                    <table class="table pad10">
                        ...
                        <?php foreach($f1['Breakdown'] as $b_i => $b) : ?>
                            <?php for($c=0;$c<$b['PassengerTypeQuantity']['Quantity'];$c++): ?>
                                ...
                                <tr>
                                    ...
                                    <td class="text-center"><input type="text" ng-model="p.<?php echo $pid.'['.$i;?>].R" class="form-control" placeholder="Remarks" style="max-width: 150px;"></td>
                                    <td class="text-center" style="font-size:12px;vertical-align: middle;" nowrap>
                                        <?php foreach($ff as $f): ?>
                                            <?php foreach($f['Breakdown'][$b_i]['FareInfos'] as $_i => $finfo): ?>
                                                <div>
                                                    <?php echo $finfo['Meal']; ?>
                                                </div>
                                            <?php endforeach; ?>
                                    ...
                                    <td class="text-center" style="font-size:12px;vertical-align: middle;" nowrap>
                                        ...
                                        <?php foreach($ff as $f): ?>
                                            <?php foreach($f['Breakdown'][$b_i]['PassengerFare']['BaggageInformation'] as $_i => $bagg): ?>
                                                <div>
                        ...
                        <table class="table pad10" >
                            ...
                            <tr>
                                <td colspan="7" style="">
                                    ...
                                    <textarea name="data1" hidden><?php echo json_encode($data1, true); ?></textarea>
                                    <input type="hidden" name="chosen_onward" value="<?php echo $chosen_onward; ?>">
                                    <input type="hidden" name="chosen_return" value="<?php echo $chosen_return; ?>">
