...
<!--                        abacus!-->
                        ...
                        <table class="table pad10">
                            ...
                            <input type="hidden" name="uploadcount" id="uploadcount" value='<?php echo $passengers['Senior']; ?>' readonly>
                            ...
                            <?php foreach($passengers as $ptype => $pcount):?>
                                <?php for($i=0;$i<$pcount;$i++): ?>
                                    ...
                                    <?php endif; ?>
                                        ...
                                        <?php if($ptype=='Senior'): ?>
                                            <td class="text-center">
                                                <input type="text" ng-model="p.<?php echo $ptype.'['.$i;?>].S" class="form-control" placeholder="Senior ID" required>
                                                <!-- <br/> -->
                                                <div class="text-left">
                                                    <label class="text-danger">Upload Senior ID Picture:</label>
                                                    <input type="file" class="form-control required btnUploadSeniorID" data-mynumber="<?php echo $no; ?>" id="<?php echo 'fileseniorid'.$no ?>" name="<?php echo 'fileseniorid'.$no ?>" title="No File Found" onchange="ValidateUploadFile(this);"  required='required' >
                                                    <input type="hidden" name="<?php echo 'seniorIdImg'.$no ?>" id="<?php echo 'seniorIdImg'.$no ?>" value='' readonly>
                                                ...
                                                <div class="form-group">
                                                    <div class="checkbox">
                                                        <label>
                                                            ...
                                                            <span class="text-danger"> 
                                                                Name and OSCA ID number on the identification card (Senior Citizen, Student, PWD, Military) should be the same with the ticket to be presented upon check-in. Failure to present ID may result to collection of the discounted rate at the airport check in counter.
                                                            </span>
                                                        </label>
                                                    </div>
                                                </div>
                        ...
                        <form method="post" action="#" class="myform">
                            ...
                            <table class="table pad10" >
                                ...
                                <tr class="pad10">
                                    <td><input type="text" ng-model="c.contactno" class="form-control" placeholder="Contact No." required></td>
                                    <td><input type="email" ng-model="c.emailadd" class="form-control" placeholder="Email Address" required></td>
                                    <?php if($this->user['R'] == 'F7997947' && $OnewayCarrierCP == '5J'):?>
                                    <td class="text-center">
                                        <select ng-model="cp.nationality" class="form-control" required>
                                ...
                                <tr>
                                    <td colspan="7" style="">
                                        <textarea name="passengers" hidden>{{p|json}}</textarea>
                                        <textarea name="contacts" hidden>{{c|json}}</textarea>
                                        <textarea name="airlineN" hidden>{{cp|json}}</textarea>
                                        <textarea name="airlineCP" hidden><?php echo json_encode($OnewayCarrierCP, true); ?></textarea>
                                        <textarea name="data" hidden><?php echo json_encode($data, true); ?></textarea>
                                        <textarea name="chosen_onward" hidden><?php echo json_encode($chosen_onward, true); ?></textarea>
                                        <textarea name="chosen_return" hidden><?php echo json_encode($chosen_return, true); ?></textarea>
                                        <textarea name="onward_baggage" hidden><?php echo json_encode($onwardbaggages,true); ?></textarea>
