<div class="mainpanel">
  ...
  <!--===================================| Built-In |=====================================================-->
	  ...
	  var fetchedData;
	  var date_from, date_to, tracking_no;
      ...
      $(document).ready(function(){//UPSBEA0010|4,UPSWEL004|4
        		...
        		//|----------------------| Add Cart |-------------------------|//
					$('#BTNAddCart').click(()=>{
						...
						if (cart != '') {
							...
							var cebuRMRegs = [
								'RMGLB', 'RMDBA', 'RMLVM', 'RMSWS'
							]

							var manilaRMRegs = [ 
								'UPS-RMRICE-0015', 'UPS-RMRICE-0014', 'UPS-RMRICE-0018', 'UPS-RMRICE-0017', 'UPS-RMRICE-0021', 'UPS-RMRICE-0061', 'UPS-RMRICE-0019', 'UPS-RMRICE-0016', 'RMSC', 'RMSP', 'RMSS', 'RMSU','UPS-RMRICE-0086','UPS-RMRICE-0087','UPS-RMRICE-0059','UPS-RMRICE-0060','UPS-RMRICE-0020'
							]
				...
				//|----------------------| Pay Out |--------------------------|//
					$('#BTNPayout').click(()=>{
						...
						if(GetValue('remarks')=='For Deliver' && onlyNCR != '1'){
								if( data[0].product_code=='UPS-RMRICE-0015'||
									data[0].product_code=='UPS-RMRICE-0014'||
									data[0].product_code=='UPS-RMRICE-0018'||
									data[0].product_code=='UPS-RMRICE-0017'||
									data[0].product_code=='UPS-RMRICE-0021'||
									data[0].product_code=='UPS-RMRICE-0061'||
									data[0].product_code=='UPS-RMRICE-0019'||
									data[0].product_code=='UPS-RMRICE-0016'||
									data[0].product_code=='UPS-RMRICE-0086'||
									data[0].product_code=='UPS-RMRICE-0087'||
									data[0].product_code=='UPS-RMRICE-0059'||
									data[0].product_code=='UPS-RMRICE-0060'||
									data[0].product_code=='UPS-RMRICE-0020'){
	  			 ...
	  			 //|----------------------| Search By Date |-------------------------|//
					...
					$('#p_date_from, #p_date_to, #o_date_from, #o_date_to').on('change', function(){
						$(this).removeAttr('style');
					})
					
					$('#p_date_from, #p_date_to, #o_date_from, #o_date_to').keydown(function (e) {
						if (e.keyCode == 13) {
							e.preventDefault();
							return false;
						}
					...
					$('#divPurchases button[name=btn_date_search], #divOrders button[name=btn_date_search]').on('click', function(){
						// alert('jusko')
						var form_data = new FormData();

						if($('#divPurchases').is(':visible')){
						form_data.append("task", "GETPURCHASES");
						date_from = $('#divPurchases input[name=p_date_from]').val();
						date_to = $('#divPurchases input[name=p_date_to]').val();
						tracking_no = $('#divPurchases input[name=p_tracking_no]').val();

							if(date_from || date_to){
								if(!date_from){
									$('#divPurchases input[name=p_date_from]').focus();
									$('#divPurchases input[name=p_date_from]').attr('style', 'border: red 1px solid;');
									return;
								}
								if(!date_to){
									$('#divPurchases input[name=p_date_to]').focus();
									$('#divPurchases input[name=p_date_to]').attr('style', 'border: red 1px solid;');
									return;
								}
							}
							GetPurchases();
						...
						if($('#divOrders').is(':visible')){
							form_data.append("task", "GETORDERS");
							date_from = $('#divOrders input[name=o_date_from]').val();
							date_to = $('#divOrders input[name=o_date_to]').val();
							tracking_no = $('#divOrders input[name=o_tracking_no]').val();

							if(date_from || date_to){
								if(!date_from){
									$('#divOrders input[name=o_date_from]').focus();
									$('#divOrders input[name=o_date_from]').attr('style', 'border: red 1px solid;');
									return;
								}
								if(!date_to){
									$('#divOrders input[name=o_date_to]').focus();
									$('#divOrders input[name=o_date_to]').attr('style', 'border: red 1px solid;');
									return;
								}
							}
							GetOrders();
						...
						// 	processData : false,
						//     contentType : false,
						// 	url: path+'/getPurchase',
						// 	success: function(result){
						// 		var jsonData = JSON.parse(result);
						// 		fetchedData = jsonData;
						// 		if($('#divPurchases').is(':visible')){
						// 			$('#PurchaseList tbody > tr').remove();
						// 		}
						...
						// 		if(jsonData.Count > 0) {
						// 			// $('#viewCart').modal('show');
						// 			jsonData.Data.forEach(function(h){
						// 				if($('#divPurchases').is(':visible')){
						// 					$(`#PurchaseList tbody`).append(`
						// 						<tr>
						// 							<td width="150" onclick="viewSelCart('${h.trackingno}')" class="w3-center">${h.date_purchased}</td>
						// 							<td width="250" onclick="viewSelCart('${h.trackingno}')" class="w3-center">${h.trackingno}</td>
						// 							<td width="200" onclick="viewSelCart('${h.trackingno}')" class="w3-center">${h.delivery_address}</td>
						// 							<td width="200" onclick="viewSelCart('${h.trackingno}')" class="w3-center">${h.contact_person}</td>
						// 							<td width="150" onclick="viewSelCart('${h.trackingno}')" class="w3-center">${h.contact_person}</td>
						// 							<td width="100" onclick="viewSelCart('${h.trackingno}')" class="w3-center">${h.total_amount}</td>
						// 							<td width="100" onclick="viewSelCart('${h.trackingno}')" class="w3-center">${h.Cart.length}</td>
						// 							<td width="150" onclick="viewSelCart('${h.trackingno}')" class="w3-center">${h.payment_type}</td>
						// 							<td width="150" onclick="viewSelCart('${h.trackingno}')" class="w3-center">${h.status}</td>
						// 						</tr>
						// 					`);
						// 				}
						...
						// 							<td width="150" onclick="viewSelCart('${h.trackingno}')" class="w3-center">${h.payment_type}</td>
						// 							<td width="150" onclick="viewSelCart('${h.trackingno}')" class="w3-center">${h.status}</td>
						// 							<td class="w3-center" id="act${h.rowid}">
						// 								<input type="button" value="Claim" class="w3-btn w3-ripple w3-green" onclick="UpdOrder('Claimed','${h.rowid}')" />
						// 							</td>
						// 						</tr>
						// 					`);
						// 				}
						...
						// 		}else {
						// 			if($('#divPurchases').is(':visible')){
						// 				$('#PurchaseList').html('<tbody><tr><td colspan="100%" class="w3-center"> <h1>No Order has been made</h1></td></tr><tbody>'); 
						// 			}
						// 			if($('#divOrders').is(':visible')){
						// 				$('#OrderList').html('<tbody><tr><td colspan="100%" class="w3-center"> <h1>No Order has been made</h1></td></tr><tbody>'); 
						// 			}
						// 		}
				...
				readCart(); 
				term();
					// REMOVE CONDITION FOR LIVE
				// if(level==7 && (R=='F8745532' || R=='F7997947')){
				if(level==7){
					view('divDisplay','divOrders');
				}
				else{
					view('divDisplay','divProducts');
				} 
	  ...
	  //|----------------------| View Cart |----------------------|//
	   	const viewCart=(ID,Type)=>{
			...
			for(x in data[0].Cart){
				...
				if(	data[0].Cart[x].product_code=='UPS-RMRICE-0015'||
					data[0].Cart[x].product_code=='UPS-RMRICE-0014'||
					data[0].Cart[x].product_code=='UPS-RMRICE-0018'||
					data[0].Cart[x].product_code=='UPS-RMRICE-0017'||
					data[0].Cart[x].product_code=='UPS-RMRICE-0021'||
					data[0].Cart[x].product_code=='UPS-RMRICE-0061'||
					data[0].Cart[x].product_code=='UPS-RMRICE-0019'||
					data[0].Cart[x].product_code=='UPS-RMRICE-0016'||
					data[0].Cart[x].product_code=='UPS-RMRICE-0086'||
					data[0].Cart[x].product_code=='UPS-RMRICE-0087'||
					data[0].Cart[x].product_code=='UPS-RMRICE-0059'||
					data[0].Cart[x].product_code=='UPS-RMRICE-0060'||
					data[0].Cart[x].product_code=='UPS-RMRICE-0020'||
					data[0].Cart[x].product_code=='RM2B'||
					data[0].Cart[x].product_code=='RM3B'||
					data[0].Cart[x].product_code=='RMBT'||
					data[0].Cart[x].product_code=='RMCS'||
					data[0].Cart[x].product_code=='RMCT'||
					data[0].Cart[x].product_code=='RMF'||
					data[0].Cart[x].product_code=='RMFL'||
					data[0].Cart[x].product_code=='RMJS'||
					data[0].Cart[x].product_code=='RMPGL'||
					data[0].Cart[x].product_code=='RMPNL'||
					data[0].Cart[x].product_code=='RMPSL'||
					data[0].Cart[x].product_code=='RMSC'||
					data[0].Cart[x].product_code=='RMSF'||
					data[0].Cart[x].product_code=='RMSP'||
					data[0].Cart[x].product_code=='RMSS'||
					data[0].Cart[x].product_code=='RMSU'){
	  ...
	  //|----------------------| View Selected Cart |----------------------|//
	  const viewSelCart=(ID)=>{
			var records=orders,x,str='',prc,gc,pv,dis,qty,wght,total=0,totalprc=0,totaldis=0,totalqty=0,totalgc=0,totalpv=0,grandtotal=0,totalpw=0;   
			var data = fetchedData.Data.filter((data)=>{
			return (data.trackingno==ID)?data:null;
				return data;
			});
			...
			$('#pdfLink').prop({
				"href":"https://mobilereports.globalpinoyremittance.com/portal/mlm_receipt/"+ID,
				"target":"_blank"});
			$('#lblTrack').html(ID);
			$('#tPrice').html(Currency(totalprc));
			$('#tGCPrice').html(Currency(totalgc));
	  	...
	  	GetOrders=()=>{
			...
			$.ajax({
				...
				success: function(result){
					...
					// console.log('Get Order Response:'+orders);
					if(orders.S==2 || orders.S==0){
						// GetOrders();
                        alert('No Orders')
					}
					else {
		...
		GetPurchases=()=>{
			...
			$.ajax({
				...
				success: function(result){
					...
					console.log('Get Purchases Response:'+purchases);
					if(purchases.S==2 || purchases.S==0){
                        // GetPurchases();
						alert('No Purchase')
					}
					else{
		...
		GetTransaction=()=>{
			...
			$.ajax({
				...
				success: function(result){
					...
					if(transaction.S==2 || transaction.S==0){
                        // GetTransaction();
		...
		const readCart=()=>{
			...
			if(cart!=''){
				...
				for(var x=0;x<itm.length;x++){
					...
					var rm25Kg = ['UPS-RMRICE-0015', 'UPS-RMRICE-0014', 'UPS-RMRICE-0018', 'UPS-RMRICE-0017', 'UPS-RMRICE-0021', 'UPS-RMRICE-0016', 'RMGLB', 'RMDBA', 'RMSWS', 'RMVLM','UPS-RMRICE-0086','UPS-RMRICE-0087','UPS-RMRICE-0060','UPS-RMRICE-0020']
					var rm50Kg = ['UPS-RMRICE-0061', 'UPS-RMRICE-0019','UPS-RMRICE-0059']

					var totalpw2 = parseFloat(
						rm25Kg.includes(data[0].product_code) ? details[1] * 25 : (rm50Kg.includes(data[0].product_code) ? details[1] * 50 : details[1] * data[0].weight)
					).toFixed(2)
			...
			var ricepackaged = txt.includes('UPS-RMRICE-0015') || txt.includes('UPS-RMRICE-0014') || txt.includes('UPS-RMRICE-0018') || txt.includes('UPS-RMRICE-0017') || txt.includes('UPS-RMRICE-0021') || txt.includes('UPS-RMRICE-0061') || txt.includes('UPS-RMRICE-0019') || txt.includes('UPS-RMRICE-0016') || txt.includes('RMSC') || txt.includes('RMSU') || txt.includes('RMSP') || txt.includes('RMSS') || txt.includes('UPS-RMRICE-0086') || txt.includes('UPS-RMRICE-0087') || txt.includes('UPS-RMRICE-0059')|| txt.includes('UPS-RMRICE-0060')|| txt.includes('UPS-RMRICE-0020');

			var riceCebuPackaged = txt.includes('RMDBA') || txt.includes('RMGLB ') || txt.includes('RMLVM ') || txt.includes('RMSWS');
			...
			if(ricepackaged == true){
				...
				console.log("LEVEL",data[0].regcode);
				// if(data[0].regcode == '1234567' || data[0].regcode == 'GRM466331'){
				// 	$('.PType').attr("hidden",false);
				// }else{
	  ...
	  //|----------------------| View Order |---------------------|//
		const viewOrder=()=>{
			...
			if(orders.Data!=null){
				...
				//data.reverse();
					// console.log("testing123", JSON.stringify(data));
				for (x in data) {
					d= new Date(data[x].date_purchased);
					person=data[x].Outlet[0].fname+' '+data[x].Outlet[0].lname;
			...
			<div class="col-md-12" id="container-cart">
				<div class="w3-orange"><b>
					...
					echo'<input type="button" class="w3-button w3-ripple" value="MLM PRODUCTS" onclick="view(\'divDisplay\',\'divProducts\')" />
						 <input type="button" class="w3-button w3-ripple" value="MY PURCHASES" onclick="view(\'divDisplay\',\'divPurchases\')"/>
						 <input type="button" class="w3-button w3-ripple" value="TRANSACTION REPORT" onclick="view(\'divDisplay\',\'divTransaction\')"/>';
					if($user['L']=='7')
					echo'<input type="button" class="w3-button w3-ripple" value="MLM PRODUCTS" onclick="view(\'divDisplay\',\'divProducts\')" />
				...
				<div class="divDisplay w3-hide" id="divProducts">
										...
										<a data-toggle="modal" onclick="<?php echo $script ?>" style="cursor: pointer;">	
											<img src="<?php echo 'https://mobilereports.globalpinoyremittance.com/assets/images/MLMProducts/'.$row['product_code'].'.jpg' ?>" 
												 ...
												 style="height:160px; width:100%; background-image: url('https://s3-ap-southeast-1.amazonaws.com/upsmlmprod/mlm/NO_IMG.PNG'); 
												 	background-size: 100px; margin:0 auto; display:block;">
											<span class="text-center" style="margin:0 auto; display:block; font-size:11px;"><b><?php echo $row['product_name'];?></b></span>
										</a>								  	 
										<p style="font-family: 'Times New Roman', Times, serif; margin-top:10px;" class="w3-hide">
					...
					<?php } ?>
						<!--=======================================| Cart List |===================================-->					
						<div class="modal fade" id="modalProductCart" data-backdrop="static" role="dialog" style="margin-top: 5%">
				...
				<div class="divDisplay w3-hide" id="divPurchases"><br/>
					<table cellspacing="0" cellpadding=0 width="100%">
					 <tr><!-- <td width="100">Date From : </td>
						...
						<td><input type="text" class="w3-input w3-border" placeholder="Search Tracking No" name="p_tracking_no" id="p_tracking_no" style="height:50px" 
									/></td> 
									<!-- onkeyup="Searcher(this,'PurchaseList')" -->
						<td>
							<label for="date_from" style="margin-bottom: 0px !important">Purchases From:</label>
							<div class="input-group">
								<span class="input-group-addon">
									<i class="fa fa-calendar" aria-hidden="true"></i>
								</span>
								
								<input class="form-control" type="date" placeholder="Orders From Date" name="p_date_from" id="p_date_from"/>
							</div>
						 </td>
						 <td>
						    <label for="p_date_to" style="margin-bottom: 0px !important">To:</label>
							<div class="input-group">
								<span class="input-group-addon">
									<i class="fa fa-calendar" aria-hidden="true"></i>
								</span>
								<input class="form-control" type="date" placeholder="To" name="p_date_to" id="p_date_to"/>
							</div>
						 </td>
						 <td style="vertical-align: bottom; margin-left: 1em"><button type="button" class="btn btn-primary" name="btn_date_search" id="">Search</button></td>
					</tr>
					</table></br>
					<table class="w3-table w3-border ">
				...
				<div class="divDisplay w3-hide" id="divOrders"><br/>
					<table cellspacing="0" cellpadding=0 width="100%">
					 <tr><!-- <td width="100">Date From : </td>
						 ...
						 <td width="100">Date To : </td>
						 <td width="200"><input type="date" class="w3-input w3-border" placeholder="Date To" style="width:90%"/></td> -->
						 <td><input type="text" class="w3-input w3-border" placeholder="Search Tracking No" style="height:50px" name="o_tracking_no" id="o_tracking_no"
									/></td>
									<!-- onkeyup="Searcher(this,'OrderList')" -->
						 <td>
					     	<label for="o_date_from" style="margin-bottom: 0px !important">Oders From:</label>
							<div class="input-group">
								<span class="input-group-addon">
									<i class="fa fa-calendar" aria-hidden="true"></i>
								</span>
								
								<input class="form-control" type="date" placeholder="Orders From Date" name="o_date_from" id="o_date_from"/>
							</div>
						 </td>
						 <td>
						    <label for="o_date_to" style="margin-bottom: 0px !important">To:</label>
							<div class="input-group">
								<span class="input-group-addon">
									<i class="fa fa-calendar" aria-hidden="true"></i>
								</span>
								<input class="form-control" type="date" placeholder="To" name="o_date_to" id="o_date_to"/>
							</div>
						 </td>
						 <td style="vertical-align: bottom; margin-left: 1em"><button type="button" class="btn btn-primary" name="btn_date_search" id="">Search</button></td>
					</tr>
					...
					<table class="w3-table w3-border ">
					  <thead><tr class="w3-indigo">
							<th class="w3-center">DATE PURCHASE</th>
							<th class="w3-center">TRACKING NO</th>
							<th class="w3-center">CUSTOMER LOCATION</th>
							<th class="w3-center">CUSTOMER</th>
							<th class="w3-center">CONTACT NO</th>
							<th class="w3-center">AMOUNT</th>
							<th class="w3-center">QUANTIY</th>
					...
					<div class="form-group">
						<select class="form-control" name="PaymentType" id="PaymentType" >
								<option value="" disabled selected>--Choose--</option>
									...
									<option class="PType" value="PV">PAYMENT VOUCHER</option>
  ...
  <!--====================================================================================================-->
    <div class="w3-hide">
		...
		<input type="hidden" id="tracking_no" name="tracking_no"/>
