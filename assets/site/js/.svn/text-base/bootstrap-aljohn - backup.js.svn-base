    ...
    $('#etisalatprod').on('click','.btnEtisalatprod',function(){
        ...
        else if(amount == null || amount == "")
        {
            ...
            $.ajax({
                        ...
                        if(data["success"] == 'SUCESSFUL') {
                           $('.mensahe').html(data['success']);
                           $('.flasher').addClass('alert-success').removeClass('alert-danger alert-info').fadeIn().delay(8000);
                           $('#etisalatprod').empty();
                           $('#etisalatprod').append(
                                    '<div class="form-group">'+
...
//SMART PAYOUT
    ...
    $('#payoutSmartMoney').on('click','.btnpayoutCancelSmartmoney1',function(){
        $('.flasher').hide();
         $('#payoutSmartMoney').empty();
         $('#payoutSmartMoney').append(                                
             '<div class="form-group">'+
   ...
   $('.loading_icon').show();
    $.ajax({
      ...
      success:function(data){ //value='+data['errmsg']+'
          ...
          if(data['error'] == 0){
             $('.inpthide').append('<input type="hidden" name="cocgen" class="cocget" value="1400003780" >');
             $('.input_emform').hide();
             $('.policy_details_u').empty();
